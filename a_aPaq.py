...
class a_aPaq:
	...
	def __init__(self, cat=2, tipo=0, arg1=0, arg2=0, arg3=0, arg4=0):
		self.category = cat
		self.tipo = tipo
		self.arg1 = arg1
		self.arg2 = arg2
	...
	def serialize(self):
		...
		if(self.tipo == 0):
			paquete = struct.pack('!BBBHI',self.category, self.tipo, self.arg1, self.arg2, self.arg3)
			paquete = paquete + self.arg4
		if(self.tipo == 1):
			paquete = struct.pack('!BBH', self.category, self.tipo, self.arg1)
		if(self.tipo == 2):
			paquete = struct.pack('!BBBH', self.category, self.tipo, self.arg1, self.arg2)
		if(self.tipo == 3):
			paquete = struct.pack('!BBBH', self.category, self.tipo, self.arg1, self.arg2)
		if(self.tipo == 4):
			paquete = struct.pack('!BBBH', self.category, self.tipo, self.arg1, self.arg2)
		...
		if(self.tipo == 8):
			paquete = struct.pack('!BBBH', self.category, self.tipo, self.arg1, self.arg2)
		if(self.tipo == 9):
			paquete = struct.pack('!BBBHH', self.category, self.tipo, self.arg1, self.arg2, self.arg3)
		if(self.tipo == 10):
			paquete = struct.pack('!BBBH', self.category, self.tipo, self.arg1, self.arg2)
		if(self.tipo == 11):
			paquete = struct.pack('!BBH', self.category, self.tipo, self.arg1)
	...
	def unserialize(self, byteP):
		...
		if(tipo == 0):  #PUT CHUNK
			...
			self.arg1 = paquete[2]  # ID de nuestro verde
			self.arg2 = paquete[3]  # ID del archivo
			self.arg3 = paquete[4]  # ID del chunk
			self.arg4 = byteP[9:tam]
		if(tipo == 1):  #HELLO
			paquete = struct.unpack('!BBH', byteP[0:])
			self.category = paquete[0]
			self.tipo = paquete[1]  #tipo
			self.arg1 = paquete[2]  # ID de nodo
		if(tipo == 2):  #EXISTS
		...
		if(tipo == 9):  # R/ a 8
			...
			self.arg1 = paquete[2]  # ID de nuestro verde
			self.arg2 = paquete[3]  # ID del archivo
			self.arg3 = paquete[4]  # ID del nodo
		if(tipo == 10):  #DELETE
			paquete = struct.unpack('!BBBH', byteP[0:])
			self.category = paquete[0]
			self.tipo = paquete[1]  #tipo
